...
from app.models.podcast_usage import PodcastUsage  # Will create this model
...
async def check_episode_quota(organization_id: str, db: Optional[AsyncSession] = None) -> bool:
    """
    ...
    # Get tier's quota limit
    quota_limit = TIER_QUOTAS.get(tier, 0)

    # Starter tier has no podcast access
    ...
    # Unlimited tiers (Premium, Enterprise)
    if quota_limit == -1:
        logger.debug(f"Organization {organization_id} has unlimited quota ({tier.value})")
        return True

    # Professional tier: check monthly usage
    if db:
        current_usage = await get_monthly_usage(organization_id, db)
    else:
        # If no db session provided, assume usage is 0 (for testing without db)
        current_usage = 0
...
async def get_remaining_quota(organization_id: str, db: Optional[AsyncSession] = None) -> Optional[int]:
    """
...
async def increment_episode_count(organization_id: str, db: AsyncSession) -> None:
    """
...
async def get_monthly_usage(organization_id: str, db: AsyncSession) -> int:
    """
