...
# ============================================================================
# STAGE TRANSITION TESTS (Drag-and-Drop Kanban Support)
# ============================================================================


def test_update_deal_stage_success(client, create_user, create_organization, create_deal_for_org):
    """User can update deal stage via PUT /api/deals/{id}/stage endpoint."""
    org = create_organization(name="Pipeline Org")
    user = create_user(email="pipeline@example.com", organization_id=str(org.id))
    deal = create_deal_for_org(
        org_id=str(org.id),
        user_id=str(user.id),
        name="Stage Transition Deal",
...
def test_update_deal_stage_invalid_stage(client, create_user, create_organization, create_deal_for_org):
    """Invalid stage values are rejected with 422."""
    org = create_organization(name="Pipeline Org")
    user = create_user(email="pipeline@example.com", organization_id=str(org.id))
    deal = create_deal_for_org(
        org_id=str(org.id),
...
def test_update_deal_stage_not_found(client, create_user, create_organization):
    """404 returned when deal does not exist."""
    org = create_organization(name="Pipeline Org")
    user = create_user(email="pipeline@example.com", organization_id=str(org.id))
    ...
    response = client.put(
        "/api/deals/nonexistent-id/stage",
        headers={"Authorization": f"Bearer mock_token_{user.id}"},
        json={"stage": "evaluation"},
    )
...
def test_update_deal_stage_different_org(client, create_user, create_organization, create_deal_for_org):
    """403 returned when user tries to update deal from different organization."""
    org1 = create_organization(name="Org 1")
    org2 = create_organization(name="Org 2")
    user1 = create_user(email="user1@org1.com", organization_id=str(org1.id))
    user2 = create_user(email="user2@org2.com", organization_id=str(org2.id))
    ...
    response = client.put(
        f"/api/deals/{deal.id}/stage",
        headers={"Authorization": f"Bearer mock_token_{user1.id}"},
        json={"stage": "evaluation"},
    )
