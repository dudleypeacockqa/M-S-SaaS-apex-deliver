...
if (Test-Path $filePath) {
    ...
    $content = $content -replace '[REDACTED - Use .env file]', '[REDACTED - Use Render Dashboard]'
    $content = $content -replace '[REDACTED - Use .env file]', '[REDACTED - Use Render Dashboard]'
...
git filter-branch --force --tree-filter 'if [ -f "docs/testing/master-admin/2025-11-21/crud-evidence/EXECUTION_GUIDE.md" ]; then sed -i "s/[REDACTED - Use .env file]/[REDACTED - Use Render Dashboard]/g" "docs/testing/master-admin/2025-11-21/crud-evidence/EXECUTION_GUIDE.md" 2>/dev/null || true; sed -i "s/[REDACTED - Use .env file]/[REDACTED - Use Render Dashboard]/g" "docs/testing/master-admin/2025-11-21/crud-evidence/EXECUTION_GUIDE.md" 2>/dev/null || true; fi' --prune-empty --tag-name-filter cat -- 1fb36d25^..HEAD 2>&1 | Select-Object -First 50
