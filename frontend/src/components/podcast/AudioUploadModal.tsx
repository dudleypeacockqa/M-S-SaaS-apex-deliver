/**
 * AudioUploadModal - Upload audio files for podcast episodes (DEV-016)
 * Supports MP3, WAV, M4A formats up to 500MB
 */
...
const AudioUploadModal: React.FC<AudioUploadModalProps> = ({
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await fetch(`/api/podcasts/episodes/${episodeId}/upload-audio`, {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      setError(`File too large (${sizeMB}MB). Maximum size: 500MB`);
      setSelectedFile(null);
      return;
    }
