...
- **Telemetry hooks**: Helper-driven denials now emit `resource_scope_violation` entries in the RBAC audit log, giving SOC 2 reviewers a forensic trail whenever a user probes a document that belongs to another deal or tenant. These events also flow through the new audit-event sink so monitoring stacks can ingest them without DB polling. Tests validate the log is produced for both documents and generated-doc versions. [`backend/app/core/ownership.py`](../backend/app/core/ownership.py), [`backend/app/services/document_generation_service.py`](../backend/app/services/document_generation_service.py), [`backend/app/services/audit_event_sink.py`](../backend/app/services/audit_event_sink.py)
- **Document-generation scoping**: Every document-generation endpoint (AI suggestions, versions, exports) now respects `require_scoped_organization_id`, allowing master admins to impersonate tenants via `X-Master-Tenant-Id` while preventing cross-org access for regular users. The payloads force the scoped organization id regardless of client input, and new tests cover both denial and impersonation success paths. [`backend/app/api/routes/document_generation.py`](../backend/app/api/routes/document_generation.py), [`backend/tests/test_document_ai_and_versions.py`](../backend/tests/test_document_ai_and_versions.py)
- **Deal lineage for generated docs**: Templates can now capture an optional `source_deal_id`, which is stored on `generated_documents` (with proper tenant validation) so downstream ownership checks can bridge into deal context later. [`backend/app/models/document_generation.py`](../backend/app/models/document_generation.py), [`backend/tests/test_document_generation_api.py`](../backend/tests/test_document_generation_api.py)
- **Deal-aware listing**: Tenants can filter generated documents by `source_deal_id` via the `/documents` endpoint, ensuring deal-level workspaces only surface content tied to the active transaction. [`backend/app/services/document_generation_service.py`](../backend/app/services/document_generation_service.py), [`backend/app/api/routes/document_generation.py`](../backend/app/api/routes/document_generation.py), [`backend/tests/test_document_generation_api.py`](../backend/tests/test_document_generation_api.py)
- **Version/export enforcement**: All document-generation version + export endpoints now require the caller to supply the originating `source_deal_id` when one exists, preventing cross-deal access to versions, restore operations, or exports. [`backend/app/api/routes/document_generation.py`](../backend/app/api/routes/document_generation.py), [`backend/tests/test_document_generation_api.py`](../backend/tests/test_document_generation_api.py#L360)
- **Workspace navigation separation**: Legacy and new workspace sidebars now hide the Master Admin portal links unless the signed-in user’s role is `master_admin`. Regression tests cover both nav components so tenant admins never see platform tooling. [`frontend/src/components/layout/Sidebar.tsx`](../frontend/src/components/layout/Sidebar.tsx), [`frontend/src/components/layout/SidebarNavigation.tsx`](../frontend/src/components/layout/SidebarNavigation.tsx), [`frontend/src/components/layout/Sidebar*.test.tsx`](../frontend/src/components/layout/)
- **Route-level isolation**: `ProtectedRoute` no longer auto-grants master admins access to tenant-admin surfaces; routes must explicitly list `master_admin` in `requiredRole`, preventing cross-role UI leakage. [`frontend/src/components/auth/ProtectedRoute.tsx`](../frontend/src/components/auth/ProtectedRoute.tsx), [`frontend/src/components/auth/ProtectedRoute.test.tsx`](../frontend/src/components/auth/ProtectedRoute.test.tsx)
 
## Next Execution Plan (BMAD-aligned)
1. **Document AI + export parity**
   - Evaluate whether existing AI suggestion/export endpoints (in `document_generation.py`) should honor tenant impersonation via `require_scoped_organization_id` and, where deals are involved, bridge to `require_document_access` by persisting the originating `deal_id`.
   - Extend coverage to document-export download paths (and related storage-service calls) so scoped org IDs are enforced end-to-end, adding pytest cases for impersonated master-admin usage and unauthorized tenant attempts.
2. **Bulk + access-log telemetry**
   - Emit RBAC/audit log entries whenever helper-driven bulk validation fails so the SOC 2 evidence trail captures probing across deals, then surface those events in dashboards/alerts and join them with document access logs.
