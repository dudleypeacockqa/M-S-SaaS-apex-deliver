...
- **2025-11-12T02:20Z — Harness fix + full pytest coverage:** Patched `backend/tests/conftest.py` to pause coverage tracing during FastAPI `TestClient` teardown (using the collector’s private pause/resume hooks), eliminating the `IndexError` crash under pytest-cov. Captured a fresh full-suite run via `cd backend && venv/Scripts/python.exe -m pytest tests --maxfail=1 --cov=backend/app --cov-report=term-missing` (750 passed / 54 skipped / 0 errors, 84% coverage) and stored the log at `backend/pytest-latest.log` for evidence.
- **2025-11-12T02:55Z — Frontend W1.2 harness adjustments:** Updated `frontend/package.json` `test` script to run Vitest in non-watch mode by default (`vitest run`) so downstream commands no longer double-append `--run`. Full coverage command `npm run test -- --coverage --maxWorkers=1 --no-file-parallelism` now boots successfully, though a complete run under the current sequential settings exceeds the 20-minute CLI timeout; targeted suites (documents, valuation, podcast) pass with coverage, confirming the harness fix. Need to investigate TaskBoard `act(...)` warnings + runtime to shorten the full run in the next loop.
