...
- ✅ Coverage artefacts captured via the PowerShell chunk runner; combined LCOV stored at `frontend/coverage/lcov.info` with summary logged in `docs/tests/2025-11-14-frontend-vitest-coverage.txt` (lines 56.1%, functions 46.9%, branches 49.7%).
- ✅ Re-ran `scripts/run_audits.ps1` after performance optimizations; new Lighthouse scores: Performance 77, Accessibility 100, Best Practices 74, SEO 97 (reports in `docs/testing/lighthouse-report.*`). Axe audit remains 0 violations (`docs/testing/axe-report.json`).
